...
describe("SongListコンポーネントのテスト", () => {
  test("楽曲がない場合、メッセージを表示すること", () => {
    render(<SongList songs={[]} url="music" errorMessage="お気に入り曲は登録されていません" />);
    expect(screen.getByText("お気に入り曲は登録されていません")).toBeInTheDocument();
  });

  test("楽曲が1つある場合、1つのSongListItemを表示すること", () => {
    render(
      <SongList songs={[mockSong]} url="music" errorMessage="お気に入り曲は登録されていません" />,
    );
  ...
  test("複数の楽曲がある場合、複数のSongListItemを表示する", () => {
    ...
    render(<SongList songs={songs} url="music" errorMessage="お気に入り曲は登録されていません" />);
    ...
    expect(links[0]).toHaveAttribute("href", "/music/111");
    expect(links[1]).toHaveAttribute("href", "/music/555");
  });
});
